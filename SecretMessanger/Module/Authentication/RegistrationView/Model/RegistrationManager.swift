...
/// Регистрация: аккаунт, захват логина и профиль — в строго заданном порядке.
class RegistrationManager {
    ...
    /// Заводит аккаунт, занимает логин в реестре и пишет профиль.
    ///
    /// Порядок обязателен: логин занимается раньше профиля, потому что правило
    /// `users/{uid}` читает реестр и не пропустит профиль с ещё не занятым именем.
    ///
    /// - Parameter login: 3–20 символов латиницы, цифр и подчёркивания; уникальность
    ///   держит не эта проверка, а запрет `create` по существующему документу реестра.
    /// - Note: проигранная гонка за логин **откатывает аккаунт** — иначе человек остался
    ///   бы с учёткой, которой нельзя пользоваться.
    ...
    /// Удаляет только что созданный аккаунт: логин ушёл другому.
