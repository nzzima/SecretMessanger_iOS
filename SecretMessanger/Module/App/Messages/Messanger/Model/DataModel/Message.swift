...
struct Message: MessageType {
    ...
    //MARK: Отметка о смене ключа — единственное сообщение, которое никто не писал.
    // Его пишет создатель группы, удаляя участника, и содержимого у него нет вовсе:
    // ни текста, ни вложения, ни шифрования. Живёт оно в ленте наравне с сообщениями,
    // потому что событие это ленты и касается — с этого места переписка запечатана
    // другим ключом.
    var isKeyNotice: Bool
    ...
    init(messageId: String, data: [String: Any]) {
        ...
        self.isKeyNotice = type == "keyRotated"
    ...
    private func cellKind(text: String) -> MessageKind {
        //MARK: Раньше всех остальных: у отметки нет ни текста, ни автора, которого
        // стоило бы показать, и ни один из видов ниже ей не подходит.
        if isKeyNotice {
            return .custom(nil)
        }
