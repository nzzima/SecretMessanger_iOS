...
class MessangerManager {
        ...
        document.getDocument { [weak self] snap, _ in
            ...
            if let snap, snap.exists {
                ...
                self.firstKey(for: chat) { extra in
                    self.write(self.newHeader(chat: chat, keys: extra, date: Date()),
                               to: document,
    ...
    //MARK: Отметка о создании группы. Не украшение: без непустого `lastMessage` группа не
    // попадает в список «Чаты», а другого входа в неё нет — из «Контактов» открывается
    // только диалог на двоих. Созданная и не подписанная группа оказывалась недостижимой
    // навсегда: не открыть, не дописать, не добавить людей и не удалить. Найдено живьём
    // 25.08.2026.
    static let groupCreatedPreview = "👥 Группа создана"

    //MARK: Схема новой шапки собрана отдельно от записи по той же причине, что и у
    // `header(preview:...)` ниже: это договор с правилами Firestore и со всеми, кто
    // документ читает, и сторожить его должны тесты, а не глаз.
    /// Шапка диалога, которого ещё не было: состав, создатель и первые ключи.
    ///
    /// - Parameters:
    ///   - keys: записи `convoKeys` и `keyVersion`, если ключ удалось запечатать.
    ///   - date: время создания; у группы попадает в документ и задаёт её место в списке.
    func newHeader(chat: Chat, keys: [String: Any], date: Date) -> [String: Any] {
        ...
        //MARK: Отметка только группе. Диалог на двоих её не получает намеренно: шапка
        // заводится при каждом открытии чата из «Контактов», и отметка превращала бы
        // список «Чаты» в историю просмотров — строка появлялась бы у всякого, чей
        // профиль открыли. Группе такое не грозит: она заводится один раз и осознанно.
        //
        //MARK: Пишется **открытым текстом**, и это не забытое шифрование. Строка
        // постоянная, одинаковая у всех групп, и скрывать в ней нечего — а вот
        // участник, которому запись ключа не досталась (не публиковал открытый ключ),
        // увидел бы вместо неё «🔒 Сообщение не расшифровано», и группа снова выглядела
        // бы сломанной. Отметка обязана читаться всегда, иначе она бессмысленна.
        // Отсутствие `lastEnc` и означает «открытым» — так же читаются диалоги,
        // начатые до появления шифрования.
        if chat.isGroup {
            payload["lastMessage"] = MessangerManager.groupCreatedPreview
            payload["date"] = date
        }
